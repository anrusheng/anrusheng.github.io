 ...
 */function LongFact(){"use strict";/**
     ...
     */function Long(low,high,unsigned){/**
         ...
         * 类型转换缓存
         */this.typeCache={};}// The internal representation of a long is the two given signed, 32-bit values.
...
/**
     ...
     * @returns {number}
     */LongPrototype.toInt=function toInt(){if(!this.typeCache.int){this.typeCache.int=this.unsigned?this.low>>>0:this.low;}return this.typeCache.int;};/**
     * Converts the Long to a the nearest floating-point representation of this value (double, 53 bit mantissa).
     * @returns {number}
     */LongPrototype.toNumber=function toNumber(){if(!this.typeCache.number){if(this.unsigned)this.typeCache.number=(this.high>>>0)*TWO_PWR_32_DBL+(this.low>>>0);this.typeCache.number=this.high*TWO_PWR_32_DBL+(this.low>>>0);}return this.typeCache.number;};/**
     * Converts the Long to a string written in the specified radix.
     * @param {number=} radix Radix (2-36), defaults to 10
     * @returns {string}
     * @override
     * @throws {RangeError} If `radix` is out of range
     */LongPrototype.toString=function toString(radix){if(!this.typeCache.string){radix=radix||10;if(radix<2||36<radix)throw RangeError('radix');if(this.isZero())this.typeCache.string='0';if(this.isNegative()){// Unsigned Longs are never negative
if(this.eq(MIN_VALUE)){// We need to change the Long value before it can be negated, so we remove
// the bottom-most digit in this base and then recurse to do the rest.
var radixLong=fromNumber(radix),div=this.div(radixLong),rem1=div.mul(radixLong).sub(this);this.typeCache.string=div.toString(radix)+rem1.toInt().toString(radix);}else this.typeCache.string='-'+this.neg().toString(radix);}// Do several (6) digits each time through the loop, so as to
// minimize the calls to the very expensive emulated div.
var radixToPower=fromNumber(pow_dbl(radix,6),this.unsigned),rem=this;var result='';while(true){var remDiv=rem.div(radixToPower),intval=rem.sub(remDiv.mul(radixToPower)).toInt()>>>0,digits=intval.toString(radix);rem=remDiv;if(rem.isZero()){this.typeCache.string=digits+result;break;}else{while(digits.length<6)digits='0'+digits;result=''+digits+result;}}}return this.typeCache.string;};/**
...
if(approxRes.isZero())approxRes=ONE;res=res.add(approxRes);rem=rem.sub(approxRem);}return res;};/**
     ...
     */LongPrototype.toBytesBE=function(){var hi=this.high,lo=this.low;return[hi>>>24&0xff,hi>>>16&0xff,hi>>>8&0xff,hi&0xff,lo>>>24&0xff,lo>>>16&0xff,lo>>>8&0xff,lo&0xff];};return Long;}globalThis.Long=LongFact();}).call(root);})(// The environment-specific global.
